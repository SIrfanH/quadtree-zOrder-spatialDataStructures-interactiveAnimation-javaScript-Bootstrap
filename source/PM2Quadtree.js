...
class qEdge {
    ...
    checkSimilarity(line) {
        ...
        if((this.point1.checkSimilarity(line.point1) && this.point2.checkSimilarity(line.point2)) ||
            (this.point1.checkSimilarity(line.point2) && this.point2.checkSimilarity(line.point1))){
                return true;
            }
        return false;
    }
...
class Boundry {
    ...
    containsLine(line) {
        ...
        // returns 4 if no point is inside the rectangle but the line passes through the rectangle
...
class Quadtree {
    ...
    subdivide() {
        ...
        // Create every node
        this.nodes[0] = new Quadtree(nW);
        this.nodes[1] = new Quadtree(nE);
        this.nodes[2] = new Quadtree(sW);
        this.nodes[3] = new Quadtree(sE);
        ...
        for (let i = 0; i < this.qEdges.length; i++) {
            ...
            for (let j = 0; j < indexes.length; j++) {
                ...
                this.nodes[indexes[j]].insertLine(this.qEdges[i]);
            }
        }
    ...
    insertLine(line) {
        ...
        // If this node does not have a vertix and qEdge then insert the line
        if(this.vertex == null && this.qEdges.length == 0){
            // Then push the line the quadtree
            let variable = this.boundry.containsLine(line);

            if (variable == 1) {
                // If the first endpoint of the line is inside this quadrant
                this.points.push(line.point1);
                this.vertex = line.point1;
                this.qEdges.push(line);
            } else if (variable == 2) {
                // If the second endpoint of the line is inside this quadrant
                this.points.push(line.point2);
                this.vertex = line.point2;
                this.qEdges.push(line);
            } else if (variable == 3) {
                // If both of the endpoints of the line is inside this quadrant
                this.points.push(line.point1);
                this.points.push(line.point2);
                this.qEdges.push(line);
            } else {
                // If no endpoint of the line is inside this quadrant but line passes from this quadrant
                this.qEdges.push(line);
            }
            this.boundry.color = 'rgba(0,0,0,0.5)';

        // If this quadrant does not have a vertex but has qEdges then check if it has points
        } else if (this.vertex == null && this.qEdges.length != 0){

            // If this quadrant does not have any point then check if qEdges have a similar vertex
            if(this.points.length == 0){

                let flag = -1;
                for(let i = 0; i<this.qEdges.length; i++){
                    if(line.haveSimilarPoints(this.qEdges[i])){
                        flag = 1;
                    }
                }
                console.log("this line is :");
                console.log(line);
                ...
                // This quadrant has points then if the new line contains that point then make that point the vertex

                let flag = -1;
                for(let i=0; i<this.points.length; i++){
                    if(this.points[i].checkSimilarity(line.point1) || this.points[i].checkSimilarity(line.point2)){
                        flag = i;
                    }
                }

                if(flag != -1){
                    this.vertex = this.points[flag];
                    this.qEdges.push(line);
                    this.boundry.color = 'rgba(0,0,0,0.5)';
                }else{
                    this.subdivide();
        ...
        // If this quadrant has a vertex and also has qEdges
        } else if(this.vertex != null && this.qEdges.length != 0){

            // Check if any endpoints of the new Line is a vertex or not
            if (this.vertex.checkSimilarity(line.point1) || this.vertex.checkSimilarity(line.point2)) {
                this.qEdges.push(line);
                this.boundry.color = 'rgba(0,0,0,0.5)';
            } else {
                this.subdivide();
...
let quadTree = new Quadtree(boundry);
...
function checkForDistance(line){
    for(let k = 0; k<lines.length; k++){
        ...
        if(line.checkSimilarity(lines[k])){
            return true;
        }
        if(line.haveSimilarPoints(lines[k])){
            return false;
        }else{
            if(distance<10){
                return true;
