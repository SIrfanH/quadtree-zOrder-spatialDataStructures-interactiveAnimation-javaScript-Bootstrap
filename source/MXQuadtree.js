...
class Line {
    ...
    constructor(x1, y1, x2, y2) {
        ...
        this.interceptY = y1 - this.slope * x1;
        this.color = 'rgba(0,0,0,1)';

        // Used inside Peiority Queue for determining Nearest Neighbor; it is calculating using distanceFromPoint function
        this.priority = null;
    }
    ...
    distanceFromPoint(point){
        ...
        let K1 = point.x - this.x1;
        let K2 = point.y - this.y1;

        // Creating vector Z from One end to another end of the Line segment where Z = ( Z1 , Z2 )
        let Z1 = this.x2 - this.x1;
        let Z2 = this.y2 - this.y1;
        ...
        if(projectionMultiple < 0){
            // If point is over the line segment then it means x1 and y2 is the nearest end of the line segment to the point 
            projX = this.x1;
            projY = this.y1;
            ...
            projX = this.x1 + projectionMultiple * Z1;
            projY = this.y1 + projectionMultiple * Z2;
        }
        
        // Calculating distance from nearest line segmetn to the point
...
// This Class represents a Axis Aligned Bounding Box (AABB) for the quadtree Nodes
class Boundry {
    ...
    containsLine(line) {
        ...
        // return 1 if one point of the line is inside boundry
        if (line.x1 < this.centerX + this.width && line.x1 >= this.centerX - this.width
            && line.y1 < this.centerY + this.height && line.y1 >= this.centerY - this.height) {
            return 1;
        }

        // return 1 if the other point of the line is inside boundry
        if (line.x2 < this.centerX + this.width && line.x2 >= this.centerX - this.width
            && line.y2 < this.centerY + this.height && line.y2 >= this.centerY - this.height) {
            return 1;
        }

        // if both y coordinate of the line is greater than the boundry then it means line is under the boundry
        if (line.y1 > this.centerY + this.height && line.y2 > this.centerY + this.height) {
            return 0;
        }
        ...
        if (line.x1 > this.centerX + this.width && line.x2 > this.centerX + this.width) {
            return 0;
        }
        ...
        if (line.slope > 0) {
            ...
            if (uppLeftDif >= 0 && uppRightDif <= 0) {
                return 2; // line passes through upper left and upper right section of the boundary
            } else if (lowLeftDif >= 0 && lowRightDif <= 0) {
                return 2; // line passes through lower left and lower right section of the boundary
            } else if (uppLeftDif >= 0 && lowLeftDif <= 0) {
                return 2; // line passes through upper left and lower right section of the boundary
            } else if (uppRightDif <= 0 && lowRightDif >= 0) {
                return 2; // line passes through upper right and lower right section of the boundary
            } else {
...
class Quadtree {
    ...
    subdivide() {
        ...
        // Create every node
        this.nodes[0] = new Quadtree(nW, this.height - 1);
        this.nodes[1] = new Quadtree(nE, this.height - 1);
        this.nodes[2] = new Quadtree(sW, this.height - 1);
        this.nodes[3] = new Quadtree(sE, this.height - 1);
    ...
    insertLine(line) {
        ...
        // If this is the root
        if (this.boundry.level == 0 && !lines.includes(line)) {
        ...
        // If this nodes does not have children call subdivide until desired height is reached
        if (this.height > 0) {
            this.subdivide();
        ...
        // Push the line object if this is a leaf node and desired height is reached. Also color the node
    ...
    draw() {
        ...
        if (this.nodes.length == 0) {
            ...
            for (let i = 0; i < this.array.length; i++) {
                if (!lines.includes(this.array[i])) {
                    lines.push(this.array[i]);
                }
            }
        } else {
    ...
    windowSearch(window) {
        ...
        if(this.array.length != 0){
            this.boundry.color = 'rgba(240, 255, 0, 0.3)'; // Yellowe
        }

        for (let i = 0; i < this.array.length; i++) {
            if (window.containsLine(this.array[i])) {
                // Coloring Lines
               this.array[i].color = 'rgba(225,0,0,0.5)' // Orange
            }
        }
        return;
    }
    ...
    nearestNeighborSearch(point, k) {
        ...
        while(nearestNodesQueue.list.length != 0){
            ...
            if(node.nodes.length == 0){
                ...
                if(node.array.length != 0){
                    ...
                    for(let j = 0; j<node.array.length; j++){
                        if(!nearestLinesQueue.list.includes(node.array[j])){
                            nearestLinesQueue.enqueue(node.array[j], node.array[j].distanceFromPoint(point));
                        }
                    }
                }
        ...
        // Dequeue the nearest lines and insert it into the nearestLines array for animation purpose
        for(let j = 0; j<k; j++){
        ...
        for(let j = 0; j<indexArray.length; j++){
            ...
            if(indexArray[j].boundry.circleOverlap(knnCircle) && indexArray[j].array.length != 0){
    ...
    drawTree() {
        ...
        if (this.array.length != 0 && this.boundry.level != 0) {
...
let quadTree = new Quadtree(boundry, treeDepth);
...
function randomeLines(){
    if (readyLines.length == 0) {
        readyLines.push(new Line(193, 240, 310, 59));
        readyLines.push(new Line(310, 59, 370, 190));
        readyLines.push(new Line(430, 450, 185, 380));
        readyLines.push(new Line(185, 380, 33, 310));
        readyLines.push(new Line(370, 190, 468, 217));
        readyLines.push(new Line(468, 217, 430, 450));
        readyLines.push(new Line(33, 310, 60, 185));
        readyLines.push(new Line(60, 185, 193, 240));
        readyLines.push(new Line(185, 380, 370, 190));
    }
...
window.onload = function init(line) {
    animate();
    ...
    let treeDepthSlider = document.getElementById('quadTreeDepthSlider');
    treeDepthSlider.addEventListener('change', function (event) {
        // Turn on and off necessary and unnecessary parts of the program
        i = 0;
        treeDepth = treeDepthSlider.value;
        quadTree = new Quadtree(boundry, treeDepth);
        isWindowSearchOn = false;
        searchWindow = null;
        isNearestNeighborSearchOn = false;
        setIndexArrayBackToNull();
        ...
        //kNearestNeighborSearchSlider.nextElementSibling.value = 0;
...
function drawLine(line, color) {
    ...
    context.stroke();
    drawPoint(line.x1, line.y1, "black");
    drawPoint(line.x2, line.y2, "black");
    context.restore();
}
...
function setIndexArrayBackToNull() {
    ...
    for (let i = 0; i < indexArray.length; i++) {
        if (indexArray[i].boundry.color != 'rgba(0,0,0,0)') {
            ...
            if(indexArray[i].array.length != 0){
            ...
            for (let j = 0; j < indexArray[i].array.length; j++) {
                indexArray[i].array[j].color = 'rgba(0,0,0,1)';
            }
        }
    }
    indexArray = [];
...
// Function for Only reseting the indexArray color
...
function windowSearchAnimMode(node) {
    if (animationMode && indexArray.length != 0 && isWindowSearchOn) {
        ...
        for (let i = 0; i < node.array.length; i++) {
            if (node.boundry.level == treeDepth && searchWindow.containsLine(node.array[i])) {
                node.array[i].color = 'rgba(225,0,0,0.5)' // Orange for Lines
            }
        }
        context.restore();
        ctxT.restore();
...
function knnAnimMode(node){
    if(indexArray.length != 0 && isNearestNeighborSearchOn && animationMode && nearestLines.length != 0){
        ...
        // If all the nearest lines are colored the stop animation
...
canvas.addEventListener('mousemove', function (ev) {
    if (isDown == true) {
        ...
        x2 = ev.offsetX;
        y2 = ev.offsetY;
        //document.getElementById('text').innerHTML = "("+x2+","+y2+")"
    }
